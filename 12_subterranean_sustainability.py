...
the previous generation, there were plants in that pot, the one immediately to
the left, and the one two pots to the right, but not in the ones immediately to
...
def parse_pots(pots):
  return [parse_pot(pot) for pot in pots]


def simulate(state, transitions, num_generations):
  for i in range(num_generations):
    next_gen = np.zeros((len(state),))
    for j in range(2, len(state) - 2):
      next_gen[j] = transitions[tuple(state[j - 2:j + 3])]
    state = next_gen
  return np.sum(np.where(state == 1)[0] - PADDING_LEN)


if __name__ == '__main__':
  with open('input/12') as file_:
    lines = file_.readlines()
  _, _, initial_state = lines[0].strip().split()
  initial_state = parse_pots(initial_state)
  padding = np.zeros((PADDING_LEN,))
  initial_state = np.concatenate((padding, initial_state, padding))

  transitions = {}
  for line in lines[2:]:
    pots, _, next_pot = line.split()
    pots = parse_pots(pots)
    next_pot = parse_pot(next_pot)
    transitions[tuple(pots)] = next_pot

  # Part 1.
  sum_pots = simulate(initial_state, transitions, num_generations=20)
  print('Sum of pots after 20 generations:', sum_pots)

  # Part 2.
  # NOTE(eugenhotaj): After 123 iterations, the pattern reaches a stable state,
  # and each new iteration adds 58 to the sum.
  sum_pots = simulate(initial_state, transitions, num_generations=123)
  sum_pots += (int(50e9) - 123) * 58
  print('Sum of pots after 50e9 generations:', sum_pots)
