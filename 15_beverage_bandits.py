...
class Actor(object):
  ...
  def __init__(self, kind, row, col, ap, rows, cols):
    self.kind = kind
    self.row = row
    self.col = col
  ...
  def adjacencies(self):
    """Returns the cells to the left, right, up, down of self."""
    adjacencies = []
    self._bounded_append(adjacencies, self.row, self.col + 1)  # Right.
    self._bounded_append(adjacencies, self.row - 1, self.col)  # Up.
    self._bounded_append(adjacencies, self.row, self.col - 1)  # Left.
    self._bounded_append(adjacencies, self.row + 1, self.col)  # Down.
    return adjacencies

  def _bounded_append(self, array, row, col):
  ...
  def tick(self, actors, dists):
    ...
    if not enemies:
      return True
    self._move(enemies, dists)
    self._attack(enemies)
    return False

  def _move(self, enemies, dists):
    row, col = self._find_destination(enemies, dists)
    if row == -1 or col == -1:
      return
    dists_ = dists[row * self._cols + col]
    min_dist = INF
    next_row, next_col = -1, -1
    for adjacency in sorted([(self.row, self.col)] + self.adjacencies()):
      row, col = adjacency
      dist = dists_[row * self._cols + col]
      if dist < min_dist:
        min_dist = dist
        next_row, next_col = row, col
    self.row, self.col = next_row, next_col

  def _find_destination(self, enemies, dists):
    positions = []
    for enemy in enemies:
      positions.extend(enemy.adjacencies())
    positions = sorted(positions)
    u = self.row * self._cols + self.col
    distances = [dists[u, p[0] * self._cols + p[1]] for p in positions]
  ...
  def _attack(self, enemies):
    ...
    for adjacency in sorted(self.adjacencies()):
      for enemy in enemies:
        if adjacency == (enemy.row, enemy.col):
          targets.append(enemy)
    targets = sorted(targets, key=lambda t: (t.hp, t.row, t.col))
    if targets:
      target = targets[0]
      target.hp -= self.ap
...
def _shortest_path(grid):
  graph = _create_graph(grid)
  return shortest_path(graph, method='D')
...
def _create_graph(grid):
  ...
  for _ in it:
    ...
    u = row * cols + col
    _connect(graph, u, grid, rows, cols, row, col + 1)  # Right.
    _connect(graph, u, grid, rows, cols, row - 1, col)  # Up.
    _connect(graph, u, grid, rows, cols, row, col - 1)  # Left.
    _connect(graph, u, grid, rows, cols, row + 1, col)  # Down.
  return graph


def _connect(graph, u, grid, rows, cols, row, col):
...
def _simulate(grid, elf_ap=3):
  ...
  for row, col in zip(*np.where(grid == 'G')):
    actors.append(Actor('G', row, col, 3, *grid.shape))

  rounds = 0
  done = False
  while True:
    actors = [actor for actor in actors if actor.hp > 0]
    actors = sorted(actors, key=lambda actor: (actor.row, actor.col))
    for actor in actors:
      grid[grid == 'E'] = '.'
      grid[grid == 'G'] = '.'
      for actor_ in actors:
        if actor_.hp > 0:
          grid[actor_.row, actor_.col] = actor_.kind
      # TODO(eugenhotaj): this is extremly terrible code. We compute
      # shortest path wayyy too many times.
      dists = _shortest_path(grid)
      done = actor.tick(actors, dists)
      if done:
        break
    if done:
      break
    rounds += 1
  return rounds, [actor for actor in actors if actor.hp > 0]


if __name__ == '__main__':
  with open('input/15') as file_:
