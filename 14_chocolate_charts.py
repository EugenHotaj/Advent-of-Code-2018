...
second, 7. Each of the two Elves has a current recipe: the first Elf starts with
the first recipe, and the second Elf starts with the second recipe.
...
current recipes' scores of 3 and 7, their sum is 10, and so two new recipes
would be created: the first with score 1 and the second with score 0. If the
current recipes' scores were 2 and 3, the sum, 5, would only create one recipe
...
NUM_TRAINING = '047801'  # Puzzle input.


def generate_recepies():
  recepies = [3, 7]
  elf_1 = 0
  elf_2 = 1
  while True:
    new_recepies = list(str(recepies[elf_1] + recepies[elf_2]))
    recepies.extend([int(i) for i in new_recepies])
    elf_1 = (elf_1 + 1 + recepies[elf_1]) % len(recepies)
    elf_2 = (elf_2 + 1 + recepies[elf_2]) % len(recepies)
    yield recepies
...
if __name__ == '__main__':
  ...
  # Part 1.
  next_10 = None
  for recepies in generate_recepies():
    if len(recepies) >= int(NUM_TRAINING) + 10:
      next_10 = recepies[int(NUM_TRAINING):int(NUM_TRAINING) + 10]
      next_10 = ''.join(str(i) for i in next_10)
      break
  print('Next 10 recepies:', next_10)

  # Part 2.
  i = 0
  for recepies in generate_recepies():
    num = ''.join([str(r) for r in recepies[i:i + len(NUM_TRAINING)]])
    if num == NUM_TRAINING:
      break
    i += 1
  print('Recepies to the left:', i)
